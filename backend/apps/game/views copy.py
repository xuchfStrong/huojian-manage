...
class ChargeViewset(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if bool(self.request.auth) and self.request.user.group.group_type == 'SuperAdmin':
            ...
            queryset = Charge.objects.filter(id=0).order_by('-update_time')
...
class ChargeSumView(generics.GenericAPIView):
    ...
    def query(self):
        ...
        if bool(self.request.auth) and self.request.user.group.group_type == 'SuperAdmin':
            ...
            queryset = Charge.objects.values("game__game_name_cn","user__username").annotate(charge_value=Sum('charge_value'))
        elif bool(self.request.auth) and self.request.user.group.group_type == 'Admin':
            qs_game = AuthGame.objects.filter(auth_id=self.request.user.auth_id).values('game_id')
            queryset = Charge.objects.filter(game_id__in = qs_game).values("game__game_name_cn","user__username").annotate(charge_value=Sum('charge_value'))
        elif bool(self.request.auth):
            queryset = Charge.objects.filter(user_id=self.request.user.id).values("game__game_name_cn","user__username").annotate(charge_value=Sum('charge_value'))
        else:
            queryset = []
        return queryset
